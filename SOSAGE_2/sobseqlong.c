...
void sobseqlong (double nf[], double N[], double x[])
{
	sobseq(nf,x); 
}
...
{
  double *nf, *N, X, Y, rnd, fX;
  int i,j, iX, iY, inc, mrows, ncols, index;
  double *x, *XX, *Img;
...
*/	    
	...
	fX = Img[mrows + index] + (Img[mrows + index + 1] - Img[mrows + index])/ 
	        (Img[index + 1] - Img[index])* (X-Img[index]);

/*
  	printf ("%e \t %d \t %e \t %e \t %e \n",  X, j, Img[j], Img[mrows + j], fX);
*/
	rnd=x[2];
