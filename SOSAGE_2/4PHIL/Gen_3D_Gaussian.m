...
% provide the emittance values in rms in SIU units 
% for example 
...
dump1 = sobseqm2(-1,N);
% X-direction
[X,XP]  = BiGaussianRP (N, sigx, sigxp, Cxxp);
%sqrt(det(cov(X,XP)))
% Y-direction
[Y,YP]  = BiGaussianRP (N, sigy, sigyp, Cyyp);

% Z-direction
[Z,DPP] = BiGaussianRP (N, sigz, sigzp, Czzp);
